...
static void killtree(struct proc* now) {
  struct proc* p;
  //Entry Point to find the direct child of sh
  for (p = ptable.proc; p < &ptable.proc[NPROC]; ++p) {
    if (p->state == UNUSED || !(p->parent))
      continue;
    if ((!now && !strncmp(p->parent->name, "sh", sizeof("sh"))) || (now && p->parent == now))
      killtree(p);
  }
  if (now) {
    now->killed = 1;
    // Wake process from sleep if necessary.
    if(now->state == SLEEPING)
    make_runnable(now);
  }
}
